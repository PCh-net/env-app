...
import SeparatorButton from '@/app/components/SeparatorButton';
...
async function fetchData() {
  ...
  setIsLoading(true);
  try {
    const url = `/api/envatoApiSearchGR?categoryName=${categoryPath}&page=${currentPage}`;
    ...
    <main className="container mx-auto p-4">
      ...
      <div className="flex items-center mt-6 mb-5 rounded-lg p-4 shadow-lg bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-lime-400 via-lime-500 to-lime-700">
        ...
        <div className='w-1/4 flex justify-center'>
          <Image src="/images/logos/EnvatoMarket-Graphicriver-Dark.png" alt="EnvatoMarket-Graphicriver-Dark" width={403} height={82} />
        </div>
      </div>
      ...
      <div className="container mx-auto p-4">
        {products.length > 0 ? (
          ...
          {products.length > 0 ? (
            products.map((product, index) => (
              <div key={index} className="mt-12 mb-5 w-12/12 transform rounded-lg bg-gray-50 px-4 py-2 shadow-lg duration-300 hover:scale-105 md:px-8 md:py-4 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-lime-200 via-lime-500 to-lime-700">
                ...
                {product.previews.icon_with_square_preview?.square_url ? (
                // <Link href={`/graphicriver/${product.id}`}>
                // <img className="w-full object-cover transform hover:scale-90 transition-transform duration-200 hover:shadow-xl hover:shadow-lime-200/70" alt={product.author_username} src={product.previews.icon_with_square_preview?.square_url} />
                // </Link>
                <div className='flex justify-center items-center'>
                  <div className="w-[400px] h-[280px] overflow-hidden relative rounded-lg">
                  <Link href={`/graphicriver/${product.id}`}>
                    <img src={product.previews.icon_with_square_preview?.square_url} alt={product.author_username} className="w-full h-full object-cover object-top"/>
                    </Link>
                  </div>
                </div>
                ) : (
          ...
          <SeparatorButton title={`Category: ${categoryPath ? replaceSlashes(categoryPath) : ''}`} subtitle={`Page: ${currentPage}`} path={`/graphicriver`} />
        </div>
      <Footer/>
    </main>
  );
