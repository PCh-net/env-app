...
return (
    <main className="container mx-auto p-4">
      <NavBar/>
      <div className="mt-6 mb-5 rounded-lg p-4 shadow-lg bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-lime-400 via-lime-500 to-lime-700">
        <div>
          ...
          <p className='text-md md:text-lg lg:text-lg mt-1 text-lime-300'>Items last week:</p>
        </div>
      </div>
      ...
      <div className="container mx-auto p-4">
        ...
        {items.length > 0 ? (
          ...
          <Link key={index} href={`/videohive/${item.id}`}>
            <div className="mt-12 mb-5 w-12/12 transform rounded-lg bg-gray-50 px-4 py-2 shadow-lg duration-300 hover:scale-105 md:px-8 md:py-4 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-lime-200 via-lime-500 to-lime-700">
              ...
              <h2 className="mt-2 text-sm md:text-md lg:text-xl md:mt-0 bg-gradient-to-tl from-lime-600 via-lime-700 to-lime-900 bg-clip-text text-transparent line-clamp-2 text-ellipsis min-h-[2rem] truncate ...">{item.item}</h2>
              <video
                width="590"
                height="332"
                controls
