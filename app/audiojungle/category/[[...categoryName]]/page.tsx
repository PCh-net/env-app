...
async function fetchData() {
  ...
  setIsLoading(true);
  try {
    const url = `/api/envatoApiSearchAJ?categoryName=${categoryPath}&page=${currentPage}`;
    ...
    <main className="container mx-auto p-4">
      ...
      <div className="flex items-center mt-6 mb-5 rounded-lg p-4 shadow-lg bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-lime-400 via-lime-500 to-lime-700">
        ...
        <div className='w-1/4 flex justify-center'>
          <Image src="/images/logos/EnvatoMarket-Audiojungle-Dark.png" alt="EnvatoMarket-Audiojungle-Dark" width={403} height={82} />
        </div>
      </div>
      ...
      <div className="container mx-auto p-4">
        {products.length > 0 ? (
          ...
          {products.length > 0 ? (
            products.map((product, index) => (
              <div key={index} className="mt-12 mb-5 w-12/12 transform rounded-lg bg-gray-50 px-4 py-2 shadow-lg duration-300 hover:scale-105 md:px-8 md:py-4 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-lime-200 via-lime-500 to-lime-700">
                ...
                {product.previews.icon_with_audio_preview.mp3_url ? (
                <audio
                  src={product.previews.icon_with_audio_preview.mp3_url}
                  controls
                ...
                <div className='flex justify-center'>
                  ...
                  <div className="flex bottom-4 right-4 w-2/4 md:w-2/4 lg:w-2/4 justify-end mt-2">
                  <Link href={`/audiojungle/${product.id}`}>
          ...
          <SeparatorButton title={`Category: ${categoryPath ? replaceSlashes(categoryPath) : ''}`} subtitle={`Page: ${currentPage}`} path={`/audiojungle`} />
        </div>
      <Footer/>
    </main>
  );
